...
// A template-lookupcallback function type
type TemplateLookupCallback func(string) (*Template, error)

// Register a template lookup callback
func RegisterTemplateLookup(fn TemplateLookupCallback) {
	template_lookups = append(template_lookups, fn)
}

// Try and find a template using the lookup callbacks
func LookupTemplate(name string) (*Template, error) {
	for _, callback := range template_lookups {
		if t, err := callback(name); err == nil {
			return t, nil
		}
	}
...
var (
	...
	template_lookups []TemplateLookupCallback
)
...
func (tmpl *Template) parsePartial(name string) (*Template, error) {
	...
	// Try and lookup the partial first
	if t, err := LookupTemplate(name); err == nil {
		return t, nil
	}
