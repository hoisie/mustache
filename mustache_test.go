...
var tests = []Test{
	...
	//lambda tests
	{`{{#Lambda}}{{#Test}}abcd{{/Test}}{{/Lambda}}`, &struct {
		Lambda func(params []string, data string) string
	}{Lambda: func(params []string, data string) string { return data }}, "{{#Test}}abcd{{/Test}}"},
	{`{{#Show}}{{#Lambda}}efgh{{/Lambda}}{{/Show}}`, &struct {
		Show   bool
		Lambda func(params []string, data string) string
	}{Show: true, Lambda: func(params []string, data string) string { return data }}, "efgh"},
	{`{{#Show}}{{#Lambda}}jklm{{/Lambda}}{{/Show}}`, &struct {
		Show   bool
		Lambda func(params []string, data string) string
	}{Show: false, Lambda: func(params []string, data string) string { return data }}, ""},
	{`{{#Lambda firstParam secondParam}}jklm{{/Lambda}}`, &struct {
		Lambda func(params []string, data string) string
	}{Lambda: func(params []string, data string) string { return params[0] + " " + params[1] }}, "firstParam secondParam"},
