...
func TestLogicless(t *testing.T) {
	...
	output := RenderFile(
		...
		map[string]interface{}{
			"users": []map[string]string{
				map[string]string{"name": "user1"},
				map[string]string{"name": "user2"},
				map[string]string{"name": "user3"},
			},
		})
	if output != expected {
		t.Fatalf("1 testlogicless expected %q got %q", expected, output)
	}
	...
	output = RenderFile(filename, map[string]interface{}{
		"users": []map[string]string{}})
	if output != expected {
		t.Fatalf("2 testlogicless expected %q got %q", expected, output)
	}
}
