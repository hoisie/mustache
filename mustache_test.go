...
func TestCallbacks(t *testing.T) {
	...
	// This is well tested above
	some_template, _ := ParseString(tests[0].tmpl)

	// A lookup key for the test callback
	key := "test_key"

	// Register a template lookup
	RegisterTemplateLookup(func(name string) (*Template, error) {

		if name != key {
			return nil, errors.New("No Template found")
		}

		return some_template, nil
	})

	template, err := LookupTemplate(key)

	if err != nil {
		t.Error("Expected key not found in the lookup")
	}

	if template.Render() != tests[0].expected {
		t.Error("Expected result not obtained from rendered template")
	}
}
